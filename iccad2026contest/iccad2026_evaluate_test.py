...
# ANSI colors for constraint debug output (terminal)
_COLOR_GREEN = "\033[32m"
_COLOR_RED = "\033[31m"
_COLOR_RESET = "\033[0m"
_CONSTRAINT_LINE_WIDTH = 72


def _print_constraint_pass_line() -> None:
    print(f"{_COLOR_GREEN}{'─' * _CONSTRAINT_LINE_WIDTH}{_COLOR_RESET}")


def _print_constraint_violation_line() -> None:
    print(f"{_COLOR_RED}{'─' * _CONSTRAINT_LINE_WIDTH}{_COLOR_RESET}")


def _log_constraint_begin(check_name: str) -> None:
    print(f"[CONSTRAINT] Checking {check_name}...")


def _log_constraint_ok(check_name: str, detail: str = "passed") -> None:
    print(f"[CONSTRAINT][{check_name}] OK — {detail}")
    _print_constraint_pass_line()


def _log_constraint_error(check_name: str, message: str) -> None:
    print(f"[CONSTRAINT][ERROR][{check_name}] {message}")
    _print_constraint_violation_line()


def _log_constraint_summary(check_name: str, violations: int) -> None:
    if violations == 0:
        _log_constraint_ok(check_name, "0 violations")
    else:
        print(f"[CONSTRAINT][{check_name}] {violations} violation(s)")
        _print_constraint_violation_line()
...
def print_infeasible_cases_report(results: List[TestResult]) -> None:
    """Print every infeasible or errored test case after a full evaluation run."""
    illegal = [
        r for r in results
        if r.error is not None or not r.is_feasible
    ]
    print("\n" + "=" * 60)
    print(f"INFEASIBLE / ILLEGAL CASES: {len(illegal)} of {len(results)}")
    print("=" * 60)
    if not illegal:
        print("  (none — all cases satisfy hard constraints)")
        return
    for r in sorted(illegal, key=lambda x: x.test_id):
        if r.error is not None:
            print(
                f"  test_id={r.test_id:3d}  blocks={r.block_count:3d}  "
                f"cost={r.cost:.4f}  ERROR: {r.error}"
            )
            continue
        parts = []
        if r.overlap_violations > 0:
            parts.append(f"overlap={r.overlap_violations}")
        if r.area_violations > 0:
            parts.append(f"area={r.area_violations}")
        if r.dimension_violations > 0:
            parts.append(f"dimension={r.dimension_violations}")
        reason = ", ".join(parts) if parts else "hard constraint (unspecified)"
        print(
            f"  test_id={r.test_id:3d}  blocks={r.block_count:3d}  "
            f"cost={r.cost:.4f}  {reason}  "
            f"(V_rel={r.violations_relative:.4f})"
        )
...
def check_overlap(positions: List[Tuple[float, float, float, float]]) -> int:
    ...
    _log_constraint_begin("overlap (hard)")
    violations = 0
    n = len(positions)
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if overlap_x > 1e-6 and overlap_y > 1e-6:
                violations += 1
                _log_constraint_error(
                    "overlap",
                    f"blocks {i} and {j} overlap: "
                    f"overlap_x={overlap_x:.6g}, overlap_y={overlap_y:.6g} "
                    f"(block {i}: x={x1:.4g}, y={y1:.4g}, w={w1:.4g}, h={h1:.4g}; "
                    f"block {j}: x={x2:.4g}, y={y2:.4g}, w={w2:.4g}, h={h2:.4g})",
                )
    _log_constraint_summary("overlap", violations)
    return violations
...
def check_area_tolerance(
    ...
    """
    _log_constraint_begin("area tolerance (hard, soft blocks)")
    violations = 0
    for i, (x, y, w, h) in enumerate(positions):
        ...
        if target_area > 0:
            ...
            if diff > tolerance:
                ...
                _log_constraint_error(
                    "area",
                    f"block {i}: actual area {actual_area:.6g} vs target "
                    f"{target_area:.6g} (relative error {diff:.4%}, "
                    f"tolerance {tolerance:.2%}); w={w:.4g}, h={h:.4g}",
                )
    _log_constraint_summary("area", violations)
    return violations
...
def check_dimension_hard_constraints(
    ...
    """
    _log_constraint_begin("fixed/preplaced dimensions & position (hard)")
    if target_positions is None or target_constraints is None:
        _log_constraint_ok("dimension", "skipped (no target data)")
        return 0
    if len(target_constraints) < block_count:
        _log_constraint_ok("dimension", "skipped (constraints too short)")
        return 0
    ...
    for i in range(min(block_count, len(positions), len(target_positions))):
        ...
        if abs(pw - tw) > tolerance or abs(ph - th) > tolerance:
            violations += 1
            kind = "preplaced" if is_preplaced else "fixed-shape"
            _log_constraint_error(
                "dimension",
                f"block {i} ({kind}): size mismatch — "
                f"got ({pw:.6g}, {ph:.6g}), expected ({tw:.6g}, {th:.6g})",
            )
            continue
        ...
        if is_preplaced:
            if abs(px - tx) > tolerance or abs(py - ty) > tolerance:
                ...
                _log_constraint_error(
                    "dimension",
                    f"block {i} (preplaced): position mismatch — "
                    f"got ({px:.6g}, {py:.6g}), expected ({tx:.6g}, {ty:.6g})",
                )

    _log_constraint_summary("dimension", violations)
    return violations
...
def evaluate_solution(
    ...
    print(f"\n[CONSTRAINT] === evaluate_solution: {block_count} blocks ===")
    ...
    if is_feasible:
        _log_constraint_ok("hard (combined)", "feasible")
    else:
        _log_constraint_error(
            "hard (combined)",
            f"INFEASIBLE — overlap={overlap_violations}, "
            f"area={area_violations}, dimension={dimension_violations}",
        )
    ...
    if target_constraints is not None and len(target_constraints) >= block_count:
        ...
        # -----------------------------------------------------------------
        _log_constraint_begin("soft constraints (fixed/preplaced shape, grouping, MIB, boundary)")
        ...
        if SHAPELY_AVAILABLE:
            ...
            if n_fixed > 0 and target_polys is not None:
                _log_constraint_begin("fixed-shape shape match (soft/debug, shapely)")
                fixed_violations = check_fixed_const(
                    torch.nonzero(fixed_const).flatten(), pred_polys, target_polys)
                if fixed_violations > 0:
                    for idx in torch.nonzero(fixed_const).flatten().tolist():
                        _log_constraint_error(
                            "fixed-shape",
                            f"block {idx}: polygon differs from target (shapely check)",
                        )
                _log_constraint_summary("fixed-shape", fixed_violations)

            if n_preplaced > 0 and target_polys is not None:
                _log_constraint_begin("preplaced shape match (soft/debug, shapely)")
                preplaced_violations = check_preplaced_const(
                    torch.nonzero(preplaced_const).flatten(), pred_polys, target_polys)
                if preplaced_violations > 0:
                    for idx in torch.nonzero(preplaced_const).flatten().tolist():
                        _log_constraint_error(
                            "preplaced",
                            f"block {idx}: polygon differs from target (shapely check)",
                        )
                _log_constraint_summary("preplaced", preplaced_violations)
            ...
            _log_constraint_begin("grouping/cluster (soft)")
            for g in range(1, n_clust_groups + 1):
                ...
                if union_result.geom_type == 'MultiPolygon':
                    ...
                    _log_constraint_error(
                        "grouping",
                        f"cluster group {g}: blocks {group_indices} form "
                        f"{len(union_result.geoms)} disconnected components "
                        f"(+{gv} violation(s))",
                    )
            _log_constraint_summary("grouping", grouping_violations)
        elif n_clust_groups > 0:
            _log_constraint_error(
                "grouping",
                "shapely not available — grouping violations not computed",
            )

        # V_mib = Σ(s_q - 1): distinct (w,h) pairs minus 1 per group.
        _log_constraint_begin("MIB same dimensions (soft)")
        ...
        for g in range(1, n_mib_groups + 1):
            ...
            mv = len(distinct_shapes) - 1
            if mv > 0:
                mib_violations += mv
                _log_constraint_error(
                    "MIB",
                    f"MIB group {g}: blocks {group_indices} have "
                    f"{len(distinct_shapes)} distinct (w,h) shapes {sorted(distinct_shapes)} "
                    f"(+{mv} violation(s))",
                )
        _log_constraint_summary("MIB", mib_violations)

        # V_boundary: 1 per block that doesn't touch its required bbox edge/corner.
        _log_constraint_begin("boundary (soft)")
        ...
        if n_boundary > 0:
            ...
            for i in range(block_count):
                ...
                if not all(touches[bit] for bit in (1, 2, 4, 8) if code & bit):
                    ...
                    failed_bits = [
                        bit for bit in (1, 2, 4, 8)
                        if code & bit and not touches[bit]
                    ]
                    bit_names = {1: "left", 2: "right", 4: "top", 8: "bottom"}
                    failed = ", ".join(bit_names[b] for b in failed_bits)
                    _log_constraint_error(
                        "boundary",
                        f"block {i}: boundary code {code} — not touching "
                        f"required edge(s): {failed} "
                        f"(block at x={bx:.4g}, y={by:.4g}, w={bw:.4g}, h={bh:.4g}; "
                        f"bbox x=[{x_min_bb:.4g},{x_max_bb:.4g}], "
                        f"y=[{y_min_bb:.4g},{y_max_bb:.4g}])",
                    )
            _log_constraint_summary("boundary", boundary_violations)
    ...
    print(
        f"[CONSTRAINT] Soft summary: boundary={boundary_violations}, "
        f"grouping={grouping_violations}, MIB={mib_violations}, "
        f"total={total_soft_violations}/{n_soft} "
        f"(V_rel={violations_relative:.4f})"
    )
    print(f"[CONSTRAINT] === evaluate_solution done: feasible={is_feasible} ===\n")
...
class ContestEvaluator:
    ...
    def evaluate(
        ...
        print_infeasible_cases_report(results)
        ...
        return EvaluationResult(
            ...
            summary={
                ...
                'num_infeasible': sum(
                    1 for r in results if r.error is not None or not r.is_feasible
                ),
...
def main():
    ...
    if args.evaluate:
        ...
        print(f"Infeasible: {result.summary.get('num_infeasible', 0)}")
